...
const WatchPage = () => {
  ...
  const getVideoDetails = async () => {
    const data = await Promise.all([fetch(YOUTUBE_VIDEO_WATCH_API + videoId), fetch(YOUTUBE_VIDEO_API)]);
    const watchVideoJson = await data[0].json();
    const relatedVideoJson = await data[1].json();
    setVideo(watchVideoJson?.items[0]);
    setRelatedVideos(relatedVideoJson?.items);
          ...
          <div className='p-2 m-2'>
            <div>
              ...
              <div className='mt-2 flex justify-between'>
                ...
                <div className='flex'>
                  <button className='bg-gray-100 rounded-l-full px-4 hover:bg-gray-200'><img alt='likeBtn' className='inline-block' src={likeIcon} /> 40K</button>
